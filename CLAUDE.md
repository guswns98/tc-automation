...
> **중요**: 전체 섹션/파일 단위 읽기는 데이터 양이 너무 많아 **실사용 불가 수준** (응답 불완전 또는 실패).
> 개별 프레임의 node-id 단위 호출은 **사용 가능한 수준**으로 확인됨.
...
### xlsx 생성 방법
- TC 데이터는 `C:/Users/rkd98/TC_Automation/tc_data/<화면명>.py` 파일에 작성한다
- 파일 내 `screen_name` (문자열)과 `tc_data` (튜플 리스트) 두 변수만 정의하면 된다
- `generate_tc.py`는 엔진 역할만 하며 직접 수정하지 않는다

...
> **전체 섹션/파일 단위 호출은 실사용 불가 수준**
> 데이터 양이 너무 많아 응답이 불완전하거나 실패함.
>
> **개별 프레임의 node-id 단위 호출은 사용 가능한 수준**
> Figma URL에서 `?node-id=123-456` 형태로 추출하여 프레임 하나씩 호출할 것.
...
```
TC_Automation/
├── generate_tc.py        # xlsx 생성 엔진 (수정 불필요)
...
2. `TC 작성해줘` 명령 입력
3. `tc_data/<화면명>.py` 파일 자동 생성
4. 스크립트 실행 → `output/` 폴더에 `QA_[화면명]_TC.xlsx` 저장

```bash
python generate_tc.py tc_data/<화면명>.py
```
